...
func Run(server *Server, pattern string) error {
	...
	router.GET("/gaffer", HandleWrapper(server, server.Overview))
	router.GET("/gaffer/:host", HandleWrapper(server, server.Overview))
	router.GET("/gaffer/:host/:service", HandleWrapper(server, server.Overview))
